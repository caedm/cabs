...
def readConfigFile():
    ...
    global settings
    with open("cabsbroker.conf", 'r') as f:
        for line in f:
            line = line.strip()
...
def main():
    ...
    try:
        db = pymysql.connect(host=dbhost, port=dbport, user=dbuser, password=dbpass, db=dbname)
        cursor = db.cursor()
    except:
        try:
            db = pymysql.connect(host=dbhost, port=dbport, user=dbuser, passwd=dbpass)
            cursor = db.cursor()
