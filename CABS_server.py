...
class HandleClient(LineOnlyReceiver, TimeoutMixin):
    ...
    def writeMachine(self, machines, user, pool, restored, secondary=False):
        if restored:
            ...
            logger.info("Restored {0} to {1}".format(stringmachine, user))
            ...
            try:
                ...
                if settings.get("Auth_Secure") == 'True':
                    try:
                        ...
                        logger.error("Could not start a TLS connection to the server at {0} with " + \
                                     "the certificate {1}".format(Server, settings.get("Auth_Cert")))
