...
from shared.clientlib import Clientlib, Reconnect, ServerError
...
if __name__ == "__main__":
    ...
    if clientlib.readConfigFile('CABS_client.conf'):
        ...
        version_err_msg, pools=clientlib.getPools(cl_user,cl_pass,settings["Host_Addr"], int(settings["Client_Port"]))
        pools_array = []
        for pool in pools:
        ...
        try:
            existing_conn, cl_machine = clientlib.getMachine(user=cl_user, password=cl_pass, pool=pools_array[cl_pool][0],
                                                             host=settings["Host_Addr"], port=int(settings["Client_Port"]),
                                                             retry=0, reconnect_preference=Reconnect.NotSpecified)
            # check the flag to see if we have an existing connection or not.
            if existing_conn:
                print("Previous connection found.")
                print("If you choose to log into a new machine in this pool, all of your existing sessions in this pool will be terminated.\nThis means that you will be logged out of all other machines in this pool, and any work that has not been saved will be lost.")
                print("reconnect to existing machine?")
                reconn = input("y/n")
                while not (reconn == "y" or reconn == "n"):
                    reconn = input("y/n")
                if reconn == "y":
                    existing_conn, cl_machine = clientlib.getMachine(user=cl_user, password=cl_pass, pool=pools_array[cl_pool][0],
                                                                     host=settings["Host_Addr"], port=int(settings["Client_Port"]), reconnect_preference=Reconnect.Do)
                elif reconn == "n":
                    existing_conn, cl_machine = clientlib.getMachine(user=cl_user, password=cl_pass, pool=pools_array[cl_pool][0],
                                                                     host=settings["Host_Addr"], port=int(settings["Client_Port"]), reconnect_preference=Reconnect.Dont)
