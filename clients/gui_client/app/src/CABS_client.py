...
settings = {}
clientlib = Clientlib(root, settings)
try:
    import psutil
...
class MainPage(wx.Panel):
    ...
    def InitUI(self):
        ...
        self.user_sizer.Add(self.user_label, 0, wx.ALIGN_CENTER_VERTICAL, 15)
        self.user_sizer.Add(self.username, 1, wx.EXPAND | wx.RIGHT, 15)

        self.pass_sizer = wx.BoxSizer(wx.HORIZONTAL)
        self.pass_sizer.Add(self.pass_label, 0, wx.ALIGN_CENTER_VERTICAL | wx.LEFT, 15)
        self.pass_sizer.Add(self.password, 1, wx.EXPAND | wx.RIGHT, 15)

        self.sizer = wx.BoxSizer(wx.VERTICAL)
        self.sizer.Add(self.user_sizer, 0, wx.EXPAND | wx.ALL, 6)
        self.sizer.Add(self.pass_sizer, 0, wx.EXPAND | wx.ALL, 6)
...
class DisplayTab(wx.Panel):
    def __init__(self, parent):
        wx.Panel.__init__(self, parent)
        self.Resolutions = [(60, 40), (320, 320), (640, 480), (800, 600), (1024, 768), (1152, 864), (1280, 768),
                            (1280, 800), (1280, 960), (1280, 1024), (1360, 768), (1600, 1200), (1680, 1050),
                            (1920, 1080), (1920, 1200), (1920, 1440), (2048, 1536), (2560, 1600)]
    ...
    def InitUI(self):
        ...
        self.rez_slider = wx.Slider(self, wx.ID_ANY, value=len(self.Resolutions), minValue=0,
                                    maxValue=(len(self.Resolutions) - 1), size=(-1, -1), style=wx.SL_HORIZONTAL)
        self.rez_choice = wx.TextCtrl(self, wx.ID_ANY, str(self.Resolutions[len(self.Resolutions) - 1][0]) + "x" + str(
            self.Resolutions[len(self.Resolutions) - 1][1]), style=wx.TE_READONLY)
        ...
        self.depth_box = wx.ComboBox(self, wx.ID_ANY, "24-bit (High)",
                                     choices=["8-bit (Low)", "16-bit (Medium)", "24-bit (High)"], style=wx.CB_READONLY)
    ...
    def updateRezValue(self, e):
        self.rez_choice.ChangeValue(str(self.Resolutions[self.rez_slider.GetValue()][0]) + "x" + str(
            self.Resolutions[self.rez_slider.GetValue()][1]))
    ...
    def rgsSettings(self):
        ...
        if self.chkfull.IsEnabled() and self.chkfull.IsChecked():
            if (self.chkspan.IsEnabled() and self.chkspan.IsChecked()) or (
                    self.chkmatch.IsEnabled() and self.chkmatch.IsChecked()):
                # Spanning enabled
                ...
                cmdargs.append(
                    "-Rgreceiver.Session.0.VirtualDisplay.PreferredResolutionWidth=" + str(self.maxWidthOneScreen))
                cmdargs.append(
                    "-Rgreceiver.Session.0.VirtualDisplay.PreferredResolutionHeight=" + str(self.maxHeightOneScreen))
        else:
            cmdargs.append("-Rgreceiver.Session.0.VirtualDisplay.PreferredResolutionWidth=" + str(
                self.Resolutions[self.rez_slider.GetValue()][0]))
            cmdargs.append("-Rgreceiver.Session.0.VirtualDisplay.PreferredResolutionHeight=" + str(
                self.Resolutions[self.rez_slider.GetValue()][1]))
...
class AudioTab(wx.Panel):
    ...
    def InitUI(self):
        ...
        self.qual_choice = wx.ComboBox(self, wx.ID_ANY, "Medium", choices=["Low", "Medium", "High"],
    ...
    def rgsSettings(self):
        ...
        if self.chkmute.IsEnabled() and self.chkmute.IsChecked():
            ...
            if self.qual_choice.IsEnabled():
                cmdargs.append(
                    "-Rgreceiver.Audio.Quality=" + self.qual_choice.GetString(self.qual_choice.GetSelection()))
...
class KeyboardTab(wx.Panel):
    ...
    def rgsSettings(self):
        ...
        if self.wincombo_rb0.IsEnabled():
            cmdargs.append(
                "-Rgreceiver.Hotkeys.IsSendCtrlAltEndAsCtrlAltDeleteEnabled=" + str(int(self.wincombo_rb0.IsChecked())))
...
class DomainAndServer(wx.Panel):
    ...
    def InitUI(self):
        ...
        self.port = wx.SpinCtrl(self, value=settings.get("Client_Port"), style=wx.SP_ARROW_KEYS, min=1024, max=65535,
                                initial=int(settings.get("Client_Port")))
...
class PickPoolDialog(wx.Dialog):
    ...
    def InitUI(self):
        ...
        for pool in sorted(self.pools):
            ...
            self.sizer.Add(wx.Button(self.panel, ID_POOL_BUTTON, label=labelstring, style=wx.BORDER_NONE), 1,
                           wx.EXPAND | wx.ALL, 3)
...
class MainWindow(wx.Frame):
    ...
    def InitUI(self):
        ...
        header = wx.StaticBitmap(p, wx.ID_ANY, headerimage.ConvertToBitmap())
    ...
    def handleSettings(self, e):
        if e.GetId() == ID_SUBMIT_BUTTON:
            ...
            try:
                ...
                dlg = wx.MessageDialog(self, e, 'Error', wx.OK | wx.ICON_ERROR)
                dlg.ShowModal()
            # except:
    ...
    def poolDialog(self, pools, username, password, server, port, version_err_msg=None):
        ...
        if pools:
            ...
            if poolchoice is None:
                return
            try:
                existing_con, machine = clientlib.getMachine(username, password, poolchoice, server, port,
                                                             reconnect_preference=Reconnect.NotSpecified)
                if existing_con:
                    ...
                    recon_dlg = wx.MessageDialog(self, message=msg, caption="",
                                                 style=wx.CENTRE | wx.YES_NO | wx.YES_DEFAULT | wx.CANCEL)
                    button = recon_dlg.ShowModal()
                    if button == wx.ID_YES:
                        existing_con, machine = clientlib.getMachine(username, password, poolchoice, server, port,
                                                                     reconnect_preference=Reconnect.Do)
                    elif button == wx.ID_NO:
                        existing_con, machine = clientlib.getMachine(username, password, poolchoice, server, port,
                                                                     reconnect_preference=Reconnect.Dont)
                    else:
                        # cancel pushed.
